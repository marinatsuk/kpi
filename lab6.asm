...
;------------------------------------------------------------------------------
MACRO M_Exit ; Макрос виходу з програми
...
bak_int0Bh_offset DW ? ; Ефективна адреса функції - стандартного обробника апаратного переривання СОМ1
bak_int0Bh_seg DW ?    ; Адреса початку сегменту  функції апаратного переривання СОМ1
...
mov di, 61h            ; Вхідний параметр функції getIntVector
...
; Перенесення апаратного переривання СОМ 2 на наш вектор - 61h
...
mov di, 61h 				; Вхідний аргумент setIntVector – новий вектор DI для СОМ1 
mov dx, bx 					; Нове зміщення процедури DX (еф. адреса)
                            ; Новий сегмент ES для процедури той же
call setIntVector           ; Виклик. Переносимо стандартну функцію обробки СОМ2 на вектор 61h

; Етап 3. Визначення нового обробника переривання з дод. функціоналом на вектор 0Bh

; Підготовка аргументів

mov di, 61h                 ; DI Вхідний аргумент - номер старого вектора пер.СОМ
mov dx, OFFSET int61h       ; DX Вхідний аргумент - еф. адреса нової проц.
mov ax, SEG int61h          
mov es, ax                  ; Завантажуємо таки ES. Оскільки mov ES, SEG int0BhES не дозволено
call setIntVector           ; Виклик 

int 61h 					; виклик переривання
...
M_Exit                      ; Макрос виходу з операційної системи
